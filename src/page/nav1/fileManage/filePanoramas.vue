<template>
  <div class="file-image">
    ...
    <div class="bottom" v-if="!deleteFlag">
      <div class="num">图片数量： {{imgNum}}</div>
      <div class="totalSize">空间占用： {{totalSize | fileSize}}</div>
      <div class="btn-group">
        <el-upload
          class="upload-demo"
          :action="this.source_Upload"
  ...
  export default{
    ...
    methods: {
      ...
      async getSourceFile(){
        ...
        /*this.imageList = data.data.list.filter((item)=>{
         return /preview/.test(item.url)
         })*/
      },
      ...
      async deleteImg(){ // 删除按钮点击
        await this.$confirm('使用到这些全景图的场景可能会失效，确认删除吗?', '提示', {
